...
def timesBetweenKilling(*datas: StatsData):
  diffs = []
  for d in datas:
    t = [ ts for ts, name in d.events if name == 'KILLED' ]
    t = [ d.latency[0]['rxts'], *t, d.latency[-1]['rxts']]
    diff = [j-i for i, j in zip(t[:-1], t[1:])]
    xx = sorted([j['rxts']-i['rxts'] for i, j in zip(d.latency[:-1], d.latency[1:])])



    recovery = [0] + sorted(xx[-(len(diff) - 1):])
    diffNoFault = [ d - r for d, r in zip(diff, recovery)]
    base = diff[0]
    perc = [ (i/base) * 100 for i in diffNoFault ]

    diffs.append( (diff, perc) )
  return diffs
